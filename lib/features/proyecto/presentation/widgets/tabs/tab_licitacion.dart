...
import 'package:myapp/services/ocds_detail_viewer.dart';
...
class _TabLicitacionState extends State<TabLicitacion> {
  int selectedView = 0; // 0: Resumen, 1: Datos Completos
  ...
  Widget build(BuildContext context) {
    ...
    if (provider.cargandoExternalData) {
      return _buildLicitacionSkeleton(isMobile);
    }
    ...
    // Two viewing modes: Summary or Complete Data
    return Column(
      children: [
        // Toggle buttons
        Padding(
            ...
            children: [
              Expanded(
                child: SegmentedButton<int>(
                  segments: const [
                    ButtonSegment(label: Text('Resumen'), value: 0),
                    ButtonSegment(label: Text('Datos Completos'), value: 1),
                  ],
        ...
        // Content
        selectedView == 0
            ? _buildSummaryView(context, provider.externalApiData!, isMobile, provider)
            : _buildCompleteDataView(provider.externalApiData!),
      ],
    );
  }

  Widget _buildSummaryView(BuildContext context, Map<String, dynamic> data, bool isMobile, DetalleProyectoProvider provider) {
    // Modalidad check for display
    final isCM = Provider.of<DetalleProyectoProvider>(context).proyecto.urlConvenioMarco?.isNotEmpty == true ||
                 Provider.of<DetalleProyectoProvider>(context).proyecto.modalidadCompra.contains('Convenio Marco');

    if (isCM) {
      return _buildConvenioMarcoSection(context, data, isMobile, provider);
    }

    return _buildMpApiSection(context, data, isMobile, provider);
  }

  Widget _buildCompleteDataView(Map<String, dynamic> data) {
    final isMobile = ResponsiveHelper.isMobile(context);
    return OCDSDetailViewer(
      data: data,
      isMobile: isMobile,
    );
  }

  Widget _buildLicitacionSkeleton(bool isMobile) {
    final pad = isMobile ? 16.0 : 24.0;
    Widget infoCard(int rows) => Container(
      ...
      child: Column(
        ...
        children: [
          Padding(
            padding: const EdgeInsets.all(16),
            child: SkeletonBox(width: 100, height: 11, radius: 4),
          ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(child: Container(margin: const EdgeInsets.all(4))),
              ],
            ),
          ),
          ...
          const SizedBox(height: 16),
          infoCard(4), // Licitacion
          infoCard(4), // Comprador
          infoCard(4), // Fechas
        ],
      ),
    );
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Padding(
            padding: const EdgeInsets.all(16),
            child: Text(title, style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
          ),
          const Divider(height: 1),
          ...visible.map((r) => Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Text(r.$2!, style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w500, color: AppColors.textBody)),
                    ),
                  ],
                ),
              )),
