...
class _CampoEditableState extends State<CampoEditable> {
  ...
  Future<void> _seleccionarFecha() async {
    ...
    if (widget.valor != null && widget.valor != '—' && widget.valor!.isNotEmpty) {
      try {
        final parts = widget.valor!.split('/');
        if (parts.length == 3) {
    ...
    if (selectedDate != null) {
      if (widget.onSave != null) {
        setState(() => _guardando = true);
        try {
          await widget.onSave!(selectedDate);
          setState(() {
            _guardando = false;
            _editando = false;
          });
          _showSuccess('Fecha actualizada');
        } catch (e) {
          setState(() => _guardando = false);
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      onTap: _guardando
          ? null
          : () {
              if (widget.tipo == TipoCampo.fecha || widget.isDate) {
                _seleccionarFecha();
              } else {
