...
abstract class ProyectoRepository {
  /// Fetch a project by its Firestore document ID.
  Future<ProyectoEntity> getProyecto(String id);

  /// Fetch a project by its contract ID (idLicitacion / idCotizacion / CM id).
  /// Returns null if not found.
  Future<ProyectoEntity?> getProyectoByContractId(String contractId);
  ...
  /// Fetch a purchase order (Orden de Compra) by ID.
  ...
  /// Add an aumento (plazo/contrato) to a project.
  Future<void> addAumento(String id, AumentoEntity aumento);

  /// Update an existing aumento in place.
  Future<void> updateAumento(String id, AumentoEntity aumento);

  /// Delete an aumento from a project.
  Future<void> deleteAumento(String id, String aumentoId);
}
